...
class CardRegisterScreenTest {
    ...
    fun 카드번호를_입력하면_하이픈이_4자리마다_추가된다() {
        // given
        val cardNumberField = composeTextRule.onNode(hasText("카드 번호") and hasSetTextAction())
    ...
    fun 만료일_입력시_구분자가_추가된다() {
        // given
        val expiryDateField = composeTextRule.onNode(hasText("만료일") and hasSetTextAction())
    ...
    fun 카드_소유자_이름의_길이는_30자_이하이다() {
        // given
        val cardOwnerField = composeTextRule.onNode(hasText("카드 소유자 이름(선택)") and hasSetTextAction())
    ...
    fun `비밀번호_입력_시_노출되지_않는다`() {
        // given
        val passwordField = composeTextRule.onNode(hasText("비밀번호") and hasSetTextAction())
        // when
